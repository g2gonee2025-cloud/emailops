...
) -> tuple[int, bool, NDArray[np.float32] | None, str | None]:
    """
    ...
    * Memory-efficient streaming for large datasets
    ...
    def map_phase(
        ...
        with executor_class(**executor_kwargs) as executor:
            # Submit all batches
            future_to_batch = {
                executor.submit(_embed_batch_worker, batch): batch for batch in batches
            }
        ...
        for batch_id, success, embeddings, error in map_results:
            ...
            if success and embeddings is not None:
                for chunk_id, embedding in zip(
                    batch.chunk_ids, embeddings, strict=False
                ):
    ...
    def run_job(
        ...
        try:
            ...
            # Map phase
            logger.info(
                f"Job {job.job_id}: Starting map phase with {self.num_workers} workers"
            )
            ...
            if failed_ids and self.max_retries > 0:
                logger.info(
                    f"Job {job.job_id}: Retrying {len(failed_ids)} failed chunks"
                )
                ...
                for _attempt in range(self.max_retries):
                    ...
                    retry_embeddings, still_failed = self.reduce_phase(
                        retry_batches, retry_results
                    )

                    embeddings_by_id.update(retry_embeddings)
                    retry_chunks = [
                        c for c in retry_chunks if c["id"] in set(still_failed)
                    ]
            ...
            if embeddings_by_id:
                logger.info(
                    f"Job {job.job_id}: Updating {len(embeddings_by_id)} chunks"
                )
    ...
    def run_tenant_reindex(
        ...
        def fetch_chunks(tid: str) -> list[dict[str, Any]]:
            with SessionLocal() as session:
                ...
                return [{"id": str(c.id), "text": c.text} for c in query.all()]
