...
def make_live_api_call(account):
    ...
    try:
        ...
        if embeddings is not None and embeddings.shape[0] == len(test_texts):
            logger.info(f"✓ SUCCESS: Got {embeddings.shape[0]} embeddings with dimension {embeddings.shape[1]}")
            logger.info(f"✓ API call completed in {api_time:.2f} seconds")
            logger.info(f"✓ First embedding vector (first 10 values): {embeddings[0][:10].tolist()}")
            return True, {
                "status": "success",
...
def main():
    ...
    try:
        ...
        logger.error(f"Error during live API test: {str(e)}")
        return 1
