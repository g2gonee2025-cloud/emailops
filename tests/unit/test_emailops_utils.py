...
class TestHtmlToText(TestCase):
    ...
    @patch('emailops.utils.BeautifulSoup', side_effect=ImportError)
    def test_html_to_text_without_beautifulsoup(self, mock_bs):
...
class TestExtractText(TestCase):
    ...
    @patch('emailops.utils.PdfReader')
...
class TestEmailParsing(TestCase):
    ...
    @patch('emailops.utils.BytesParser')
    ...
    @patch('emailops.utils.extract_msg')
    def test_extract_msg_success(self, mock_extract_msg):
