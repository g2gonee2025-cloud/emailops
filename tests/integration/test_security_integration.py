...
from emailops.config import EmailOpsConfig, get_config, reset_config
    ...
    def test_symlink_security_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                is_valid, message = validate_file_path(str(symlink_file))
                assert is_valid is True
            except OSError:
...
class TestSecurityWorkflowIntegration:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for user_input in user_inputs:
                ...
                if is_valid:
                    ...
                    is_valid_ext, ext_message = validate_file_path(
                        sanitized, 
                        must_exist=False,
    ...
    def test_configuration_initialization_security(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {
                ...
                is_valid, message = validate_project_id(config.GCP_PROJECT)
                assert is_valid is False
    ...
    def test_complete_security_validation_pipeline(self):
        ...
        is_valid_file, file_msg = validate_file_path(
            sanitized_file, 
            must_exist=False,
        ...
        # 4. Validate command
        is_valid_cmd, cmd_msg = validate_command_args(
            user_command, 
            user_args,
        ...
        for name, value in env_vars:
            is_valid_env, env_msg = validate_environment_variable(name, value)
            if is_valid_env:
