...
export const request = async <T>(
  ...
  // Use a default headers object if none is provided in options
  const defaultHeaders = { 'Content-Type': 'application/json' };

  // For requests that include authentication, get the auth headers.
  // This will include the Content-Type as well.
  const authHeaders = includeAuth ? getHeaders() : {};

  const config: RequestInit = {
    ...options,
    // Spread existing headers from options, then our default/auth headers.
    // This allows overriding the Content-Type for specific requests (like login).
    headers: {
      ...defaultHeaders,
      ...authHeaders,
      ...options.headers,
  ...
  try {
    ...
    if (error instanceof ApiError) {
      // Don't re-dispatch if it's a non-401 API error, as it was already dispatched.
      if (error.status !== 401) {
        throw error;
      }
    }
