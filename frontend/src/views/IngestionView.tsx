...
function UploadSection() {
  const [documents, setDocuments] = useState<{ id: string, text: string; metadata: string }[]>([{ id: crypto.randomUUID(), text: '', metadata: '' }]);
  const [isUploading, setIsUploading] = useState(false);
  const [result, setResult] = useState<{ success: boolean; message: string } | null>(null);

  const addDocument = () => {
    setDocuments(prev => [...prev, { id: crypto.randomUUID(), text: '', metadata: '' }]);
  };
  ...
  const updateDocument = (id: string, field: 'text' | 'metadata', value: string) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setDocuments([{ id: crypto.randomUUID(), text: '', metadata: '' }]);
    } catch (err) {
      ...
      <header className="p-6 border-b border-white/5">
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center text-sm text-white/60">
              <input
                type="checkbox"
                checked={dryRun}
