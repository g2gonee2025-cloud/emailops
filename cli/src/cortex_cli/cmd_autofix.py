...
def _apply_magic_number_fixes(content: str, fixes: list, rel_path: str) -> tuple[str, list[str]]:
...
def fix_magic_numbers() -> int:
    ...
    for rel_path, fixes in MAGIC_NUMBER_FIXES.items():
        ...
        content, constants_to_add = _apply_magic_number_fixes(original_content, fixes, rel_path)
...
def _fix_return_type_in_text_extraction() -> int:
    ...
    if "def _is_cache_valid(" in content and "-> bool" not in content.split("def _is_cache_valid(")[1].split(":")[0]:
        content = re.sub(
