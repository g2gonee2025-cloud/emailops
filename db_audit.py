    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if not result:
                ...
                print("-" * 56)
                for row in result:
                    print(f"{row.table_name:<30} | {row.row_count:<10} | {row.total_size:<10}")
            ...
            if 'chunks' in tables:
                null_embeds = conn.execute(text("SELECT count(*) FROM chunks WHERE embedding IS NULL")).scalar()
                total_chunks = conn.execute(text("SELECT count(*) FROM chunks")).scalar()
                print(f"Chunks: {total_chunks} total, {null_embeds} with NULL embeddings")
