"""
Core type definitions for EmailOps error handling.
...
def collect_results(results: list[Result[T, E]]) -> Result[list[T], E]:
    ...
    for r in results:
        if not r.ok:
            return Result.failure(r.error)  # type: ignore[arg-type]
...
def traverse_results(
    ...
    if failures:
        return Result.failure(failures)  # type: ignore[arg-type]
