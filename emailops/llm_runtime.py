            ...
            if cp.exists():
                ...
                if vertexai:
                    try:
                        ...
                        logger.error("Failed to initialize Vertex AI with ADC: %s", init_err)
                        raise
        else:
            if vertexai:
                try:
                    ...
                    logger.error("Failed to initialize Vertex AI with default credentials: %s", init_err)
                    raise
        with _INIT_LOCK:
...
def _vertex_model(system_instruction: str | None = None):
    ...
    if not GenerativeModel:
        raise ImportError("Vertex AI GenerativeModel is not available. Please install google-cloud-aiplatform.")
...
def complete_text(
    ...
    if not GenerationConfig:
        raise ImportError("Vertex AI GenerationConfig is not available. Please install google-cloud-aiplatform.")
                ...
                try:
                    ...
                    if callable(batch_fn):
                        try:
                            ...
                            _check_rate_limit()
                            resp = batch_fn(
                                model=embed_name, requests=requests, config=cfg
                            )  # type: ignore
                            items = (
