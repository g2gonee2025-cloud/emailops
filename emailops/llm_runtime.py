...
def load_validated_accounts(
    ...
    if vf.exists():
        try:
            ...
            logger.info("Loaded %d validated accounts from %s", len(account_list), validated_file)
    ...
    for acc in accounts:
        ...
        if not p.is_absolute():
            p = Path(__file__).resolve().parent.parent / acc.credentials_path
        if not p.exists():
            logger.warning("Credentials file not found for %s: %s", acc.project_id, acc.credentials_path)
            acc.is_valid = False
        else:
...
def _init_vertex(
    ...
    location = location or os.getenv("GCP_REGION", "global")
    if not project:
        raise LLMError(
    ...
    try:
        if service_account_path:
            ...
            if cp.exists():
                ...
                logger.warning("GOOGLE_APPLICATION_CREDENTIALS points to %s but file not found; using ADC.", service_account_path)
...
def _ensure_projects_loaded() -> None:
    ...
    try:
        ...
        _PROJECT_ROTATION["projects"] = [
            {"project_id": a.project_id, "credentials_path": a.credentials_path} for a in accounts
        ]
...
def complete_text(
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            cfg: Dict[str, Any] = dict(max_output_tokens=max_output_tokens, temperature=temperature)
            if stop_sequences:
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if (attempt == attempts) or (not _is_retryable_error(e)):
                ...
                out = complete_text(system, user, max_output_tokens=max_output_tokens, temperature=temperature, stop_sequences=stop_sequences)
        ...
        for i in range(0, len(texts), B):
            ...
            for _ in range(max(1, len(_PROJECT_ROTATION["projects"]) or 1)):
                ...
                try:
                    ...
                    if resp and getattr(resp, "embeddings", None) and len(resp.embeddings) == len(chunk):
                    ...
                    _rotate_to_next_project(); _init_vertex()
                except Exception as e:
                    if _should_rotate_on(e) and _PROJECT_ROTATION["projects"]:
                        logger.warning("Quota on batch; rotating project"); _rotate_to_next_project(); _init_vertex(); continue
                    logger.exception("Vertex (gemini) embedding failed: %s", e)
                    break
            if not embedded:
                logger.error("All embedding attempts failed for batch starting at %d; zero vectors", i)
    ...
    for i in range(0, len(texts), B):
        ...
        for _ in range(max(1, len(_PROJECT_ROTATION["projects"]) or 1)):
            try:
                ...
                from vertexai.language_models import TextEmbeddingModel, TextEmbeddingInput  # legacy path
                ...
                if _should_rotate_on(e) and _PROJECT_ROTATION["projects"]:
                    _rotate_to_next_project(); _init_vertex(); continue
...
def _embed_azure_openai(texts: List[str], model: Optional[str] = None) -> np.ndarray:
    ...
    client = AzureOpenAI(api_key=api_key, api_version=os.getenv("AZURE_OPENAI_API_VERSION", "2024-02-01"), azure_endpoint=str(endpoint))
    vectors: List[List[float]] = []
    B = int(os.getenv("EMBED_BATCH", "100"))
    for i in range(0, len(texts), B):
        chunk = texts[i:i+B]
...
try:
    ...
    from .utils import (
        read_text_file, extract_text, clean_email_text, extract_email_metadata,
        split_email_thread, find_conversation_dirs, load_conversation, ensure_dir,
    )
except Exception:
