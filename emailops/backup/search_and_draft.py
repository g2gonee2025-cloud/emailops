...
from .index_metadata import load_index_metadata, validate_index_compatibility
...
def _resolve_effective_provider(ix_dir: Path, requested_provider: str) -> str:
    ...
    if indexed and indexed != "vertex" and not ALLOW_PROVIDER_OVERRIDE:
        raise RuntimeError(
            ...
            "Rebuild the index with gemini-embedded-001 or set ALLOW_PROVIDER_OVERRIDE=1 (unsafe)."
        )
    return "vertex"
...
def _search(
    ...
    if embs is not None:
        ...
        for pos, local_i in enumerate(top_local_idx.tolist()):
            ...
            try:
                ...
                text = ""
            item["text"] = _window_text_around_query(
    ...
    if not faiss_index_path.exists():
        logger.error(
            "Neither embeddings.npy nor %s found. Cannot search.", faiss_index_path
        )
        return []
    ...
    if q.ndim != 2 or (index_dim is not None and q.shape[1] != index_dim):
        if effective_provider != index_provider:
            try:
                ...
                logger.error(
                    "Re-embed with index provider '%s' failed: %s", index_provider, e
                )
                return []
    if q.ndim != 2 or (index_dim is not None and q.shape[1] != index_dim):
    ...
    distances, indices_array = index.search(np.ascontiguousarray(q), top_n)
    initial_scores = distances[0]
    ...
    for j, idx in enumerate(cand_indices.tolist()):
        ...
        if (
            conv_id_filter
            and str(mapping[idx].get("conv_id") or "") not in conv_id_filter
        ):
            continue
    ...
    boosted_scores = _boost_scores_for_indices(
        mapping, cand_indices, cand_scores, datetime.now(timezone.utc)
    )
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        choices=["vertex"],
        default="vertex",
        help="Embedding/search provider (Vertex only; optimized for gemini-embedded-001).",
    )
    ap.add_argument(
        "--sender",
