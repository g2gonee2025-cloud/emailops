...
def _try_load_json(s: str) -> dict[str, Any] | None:
    ...
    # 2) fenced blocks (iterate; balance-aware inside the fence)
    #    This handles code fences with additional braces in strings, etc.
    for m in re.finditer(r"```(?:json)?\s*([\s\S]*?)\s*```", s, flags=re.IGNORECASE):
        candidate = None
        try:
            # If a helper to extract balanced JSON exists, prefer it, otherwise
            # fall back to simple brace search within the fence.
            candidate_fn = globals().get("_extract_first_balanced_json_object")
            if callable(candidate_fn):
                candidate = candidate_fn(m.group(1))
            else:
                # Fallback: find first {...} span inside the fence
                b = re.search(r"\{[\s\S]*?\}", m.group(1))
                if b:
                    candidate = b.group(0)
        except Exception:
            candidate = None
    ...
    # 3) balanced braces (non-greedy, quote-aware) - prefer helper if available
    candidate_fn = globals().get("_extract_first_balanced_json_object")
    block = None
    if callable(candidate_fn):
        try:
            block = candidate_fn(s)
        except Exception:
            block = None
    if not block:
        b = re.search(r"\{[\s\S]*?\}", s)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="json",
        help="Write JSON (always) and optionally Markdown",
    )
    ap.add_argument(
        "--no-json",
        action="store_true",
