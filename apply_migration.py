...
def apply():
    ...
    print(f"Applying migrations to: {config.database.url.split('@')[-1]}") # redact password

    # We must run this from the backend directory so alembic.ini is found
    cwd = os.path.join(os.getcwd(), 'backend', 'migrations')
    ...
    os.chdir(os.path.join(original_cwd, 'backend', 'migrations'))
