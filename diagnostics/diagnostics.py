...
from diagnostics.utils import setup_logging, get_index_path, save_json_report, format_timestamp
...
REQUIRED_FIELDS = [
    "id", "path", "conv_id", "doc_type", "subject", "snippet", "modified_time"
]
...
def verify_index_alignment(root: str) -> None:
    ...
    for i, m in enumerate(mapping):
        for k in REQUIRED_FIELDS:
            if k not in m:
                fail(f"Missing field '{k}' at mapping row {i} (id={m.get('id','<no-id>')})")
        did = str(m["id"]).strip()
        if did in ids:
        ...
        try:
            ...
            if "actual_dimensions" in md:
                if int(md["actual_dimensions"]) != int(embs.shape[1]):
                    fail(f"meta.json actual_dimensions={md['actual_dimensions']} != embeddings dim={embs.shape[1]}")
        except Exception as e:
...
def check_index_consistency(root: Path) -> Dict[str, Any]:
    ...
    if not (mp.exists() and ep.exists()):
        results["recommendations"].append("Rebuild the index using the indexing process")
        return results
