...
def test_cortex_error_initialization():
    ...
    err = CortexError(
        "Test message", error_code="TEST_CODE", context={"foo": "bar"}, extra="baz"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            EmbeddingError,
            {"message": "Embedding failed", "retryable": True},
            {"retryable": True},
        ),
        (
            ValidationError,
            {"message": "Invalid input", "field": "email", "rule": "required"},
            {"field": "email", "rule": "required"},
        ),
        (
            ProviderError,
            {"message": "Provider issue", "provider": "openai", "retryable": False},
        ...
        (
            ...
            {"file_path": "/tmp/test", "operation": "read"},
        ),
        (
            TransactionError,
            {"message": "Transaction failed", "transaction_id": "123"},
            {"transaction_id": "123"},
        ),
        (
            SecurityError,
            {"message": "Unauthorized access", "threat_type": "injection"},
        ...
        (
            ...
            {"schema_name": "user_profile", "raw_output": "...", "repair_attempts": 3},
        ),
        (
            RetrievalError,
            {"message": "Could not retrieve", "query": "test query"},
            {"query": "test query"},
        ),
        (
            RateLimitError,
            ...
            {"provider": "anthropic", "retryable": True, "retry_after": 60},
        ),
        (
            CircuitBreakerOpenError,
            {"message": "Circuit open", "provider": "test", "reset_at": 12345},
            {"provider": "test", "retryable": False, "reset_at": 12345},
        ),
        (
            PolicyViolationError,
            {
...
def test_rate_limit_error_kwargs():
    ...
    err = RateLimitError(
        "Rate limit exceeded", **{"provider": "openai", "retry_after": 30}
    )
...
def test_circuit_breaker_open_error_kwargs():
    ...
    err = CircuitBreakerOpenError(
        "Circuit open", **{"provider": "test", "reset_at": 12345}
    )
...
def test_policy_violation_error_kwargs():
    ...
    err = PolicyViolationError(
        "Policy violation", **{"action": "delete", "policy_name": "admin_only"}
    )
