...
def scan_and_refresh(root: Path) -> ManifestValidationReport:
    ...
    for conv_dir in root.iterdir():
        ...
        if not attachments_dir.exists():
            ...
            report.problems.append(Problem(folder=folder_rel, issue="attachments_dir_created"))
        ...
        if manifest_path.exists():
            try:
                ...
                report.problems.append(Problem(folder=folder_rel, issue="manifest_corrupt"))

        # Build new manifest
        # Note: In a real implementation, we would parse Conversation.txt to get
        # accurate counts and dates. For B1 scope, we focus on structure and idempotency.
        # We preserve existing metadata if valid, or initialize defaults.
        ...
        new_manifest = {
            ...
            "started_at_utc": existing_manifest.get("started_at_utc", _now_iso()),
            "ended_at_utc": existing_manifest.get("ended_at_utc", _now_iso()),
        ...
        if _manifests_differ(existing_manifest, new_manifest):
            try:
                ...
                report.problems.append(Problem(folder=folder_rel, issue="manifest_write_failed"))
...
def _manifests_differ(old: Dict[str, Any], new: Dict[str, Any]) -> bool:
    ...
    keys = [
        "manifest_version", "folder", "sha256_conversation", 
        "paths", "attachment_count"
    ]
    for k in keys:
