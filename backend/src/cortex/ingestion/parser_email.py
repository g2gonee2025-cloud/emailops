...
_RE_FWD_PATTERN = re.compile(r"^(?:re|fwd?|fw|aw|sv|vs|antw|odp|回复|答复|轉寄):\s*", re.IGNORECASE)
...
def parse_eml(content: bytes) -> ParsedEmail:
    ...
    * Attachments
    ...
    headers = {}
    for hdr in ["Message-ID", "In-Reply-To", "References", "Subject", "From", "To", "Cc", "Date"]:
        val = msg.get(hdr)
        if val:
            headers[hdr] = str(val)
            ...
            if content_type == "text/plain" and "attachment" not in content_disposition:
                try:
                    ...
                    if isinstance(payload_bytes, bytes):
                        body_plain += payload_bytes.decode(_get_charset(part), errors="replace")
                ...
                try:
                    payload_bytes = part.get_payload(decode=True)
                    if isinstance(payload_bytes, bytes):
                        html_content = payload_bytes.decode(_get_charset(part), errors="replace")
...
class EmailThreader:
    ...
    def _find_thread_by_clustering(self, email: ParsedEmail) -> tuple[str | None, float]:
        """
    ...
    def thread_email(self, email: ParsedEmail) -> ThreadInfo:
        ...
        thread = self.threads[thread_id]
        position = thread.message_ids.index(email.message_id) if email.message_id in thread.message_ids else 0
    ...
    def thread_emails(self, emails: list[ParsedEmail]) -> list[tuple[ParsedEmail, ThreadInfo]]:
        """
