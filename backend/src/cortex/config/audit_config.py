...
def get_project_root() -> Path:
    ...
    try:
        ...
        while not (current_path / "backend").is_dir() or not (current_path / "frontend").is_dir():
            if current_path.parent == current_path:
                # Reached the filesystem root without finding the project structure.
                raise FileNotFoundError("Could not find the project root directory containing both 'backend' and 'frontend'.")
        ...
        if (cwd / "backend").is_dir() and (cwd/"frontend").is_dir():
            return cwd
    raise FileNotFoundError("Could not determine project root. Ensure script is run from within the project.")
...
def setup_sys_path() -> Path:
    ...
    try:
        ...
        if not src_path.is_dir():
            raise FileNotFoundError(f"Source directory not found at expected path: {src_path}")
        ...
        print("Please ensure the script is run from within the project structure.", file=sys.stderr)
        sys.exit(1)
...
def parse_env_file(path: Path) -> Dict[str, str]:
    ...
    env_vars = {}
    if not path.is_file():
        print(f"Warning: .env file not found at '{path}'. Continuing without it.", file=sys.stderr)
        return env_vars
...
def _determine_status(
    model_def: Optional[Dict], env_val: Optional[str], code_default: str
) -> str:
...
def _should_print_entry(status: str, key: str) -> bool:
    ...
    if status == "MISMATCH":
        return True
    if status == "ENV_ONLY" and key.isupper() and ("OUTLOOKCORTEX_" in key or "EMAILOPS_" in key):
        return True
    return False


def _get_env_value(env_vars: Dict[str, str], key: str) -> Optional[str]:
