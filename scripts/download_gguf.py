...
def main():
    ...
    try:
        ...
        print(f"Error: Failed to download model '{FILENAME}' from '{MODEL_REPO}': {e}", file=sys.stderr)
        print("Please check the model repository and your network connection.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        if symlink.is_symlink():
            ...
            if not symlink.resolve().exists() or symlink.resolve() != cached_path.resolve():
            ...
            print(f"Error: A file or directory already exists at '{symlink}' and is not a symlink.", file=sys.stderr)
        ...
        print(f"Error: Could not create or update symlink at '{symlink}': {e}", file=sys.stderr)
        sys.exit(1)
