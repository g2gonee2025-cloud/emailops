...
def main():
    ...
    if not db_url:
        print(
            "❌ ERROR: OUTLOOKCORTEX_DB_URL environment variable not set.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        safe_display_url = (
            f"{parsed_url.hostname}:{parsed_url.port or 5432}/{parsed_url.path[1:]}"
        )
        ...
        print(
            "❌ Connection failed: Could not connect to the database.", file=sys.stderr
        )
        print(
            "   Please check the OUTLOOKCORTEX_DB_URL and network connectivity.",
            file=sys.stderr,
        )
        sys.exit(1)
