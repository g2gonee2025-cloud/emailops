...
try:
    ...
    raise RuntimeError("Could not determine repository root from script path.")
...
if _backend_src.is_dir():
    ...
    if _backend_src_str not in sys.path:
        sys.path.append(_backend_src_str)
else:
...
def test_conversation_summarizer():
    ...
    with patch("cortex.intelligence.summarizer.LLMRuntime") as MockLLM:
        ...
        if summary != "This is a summary of the project discussion.":
            raise ValueError(f"Unexpected summary: {summary}")

        mock_instance.complete_text.assert_called_once()

        # Test embedding
        emb = summarizer.embed_summary(summary)
        if emb.tolist() != [0.1, 0.2, 0.3]:
            raise ValueError(f"Unexpected embedding: {emb}")
...
def verify_db_schema():
    ...
    if not hasattr(Chunk, "is_summary"):
        raise ValueError("Chunk model missing is_summary column")
    if not hasattr(Chunk, "chunk_type"):
        raise ValueError("Chunk model missing chunk_type column")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Verification failed with an exception:")
        sys.exit(1)
