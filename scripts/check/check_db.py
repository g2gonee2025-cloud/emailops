...
def main():
    ...
    try:
        ...
        print(
            f"Error: Failed to import a required module: {type(e).__name__}",
            file=sys.stderr,
        )
        print(f"Detail: {e}", file=sys.stderr)
        print(
            "Please ensure you have installed the project dependencies from 'requirements.txt'.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        if (
            not config
            or not hasattr(config, "database")
            or not getattr(config.database, "url", None)
        ):
            print(
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            try:
                ...
                print(
                    "Could not retrieve migration version. The 'alembic_version' table may not exist or be accessible.",
                    file=sys.stderr,
                )
                sys.exit(1)
        ...
        print(f"Error Type: {type(e).__name__}", file=sys.stderr)
        print(
            "Could not connect to the database. Verify the URL, credentials, and network access.",
            file=sys.stderr,
        )
        sys.exit(1)
