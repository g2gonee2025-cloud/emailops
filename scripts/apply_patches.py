                ...
                if line.startswith("--- ") or line.startswith("+++ "):
                    ...
                    if len(parts) > 1:
                        ...
                        if path_str.startswith("/") or ".." in Path(path_str).parts:
                            print(f"\n[SECURITY] Unsafe path detected in {patch_file.name}: '{path_str}'", file=sys.stderr)
                            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Apply all patches in the patches directory.")
    parser.add_argument(
        "-p", "--strip",
        type=int,
    ...
    if not PATCH_EXECUTABLE:
        print("Error: 'patch' command not found in PATH. Please install it.", file=sys.stderr)
        sys.exit(1)
    ...
    patches = sorted(list(PATCHES_DIR.glob("*.diff")) + list(PATCHES_DIR.glob("*.patch")))
