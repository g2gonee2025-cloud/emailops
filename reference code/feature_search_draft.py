...
if "validate_file_result" not in globals():
    # Local, conservative fallback - avoid external dependencies.
    ...
    def validate_file_result(
        ...
        """Basic validation for a local file path.

        This fallback is intentionally conservative:
          - requires an existing file by default
          - optionally rejects '..' path traversal patterns
        """
        try:
        ...
        if must_exist and not p.is_file():
            return Result.failure("Path is not a file")
    ...
    *,
    ...
    for c in context_snippets or []:
        try:
            ...
            if not result.ok:
                logger.warning(
                    "Skipping invalid attachment path: %s - %s", p, result.error
                )
                continue
        ...
        with log_timing("search_core", k=k, n_allow=len(sub_mapping)):
            ...
            for _pos, local_i in enumerate(top_local_idx.tolist()):
                ...
                try:
                    ...
                    if not result.ok:
                        logger.warning(
                            "Skipping invalid path in search: %s - %s",
                            path,
                            result.error,
                        )
...
def _search(*args, **kwargs):
    """Backward-compatible facade preserved for CLI/operations imports."""
