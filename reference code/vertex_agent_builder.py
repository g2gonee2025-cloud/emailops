...
class VertexAgentBuilder:
    ...
    def __init__(
        ...
        # Ensure the Discovery Engine client library is available
        if discoveryengine is None:
            raise ProviderError(
                "google-cloud-discoveryengine is not installed. Install it to use Vertex Agent Builder.",
        ...
        try:
            ...
            self._conversational_search_client = (
                discoveryengine.ConversationalSearchServiceClient()
            )
            self._document_service_client = discoveryengine.DocumentServiceClient()
            # Some APIs (e.g., GetDataStore) live on DataStoreServiceClient in many library versions.
            self._datastore_service_client = getattr(
                discoveryengine, "DataStoreServiceClient", None
            )
            self._datastore_service_client = (
                self._datastore_service_client()
                if callable(self._datastore_service_client)
                else None
            )
...
def _normalize_session_name(self, session: str | None) -> str | None:
    """
    Normalize a session identifier into the full resource name expected by the API.

    Accepts:
      - None (returns None)
      - Full session resource name (returned as-is)
      - Short session id (converted to full name under the configured datastore)

    NOTE: This is best-effort. Some API methods require sessions to be created server-side.
    """
    if not session:
        return None
    s = str(session).strip()
    ...
    if "/sessions/" in s:
        return s
    if not self.datastore_id:
        # Cannot build a full name without datastore context
        return s
    return (
        f"projects/{self.project_id}/locations/{self.location}"
        f"/collections/{self.collection_id}/dataStores/{self.datastore_id}"
...
def _safe_float(v: Any, default: float = 0.0) -> float:
    ...
    def answer_query(
        ...
        *,
        ...
        logger.debug(
            "answer_query request prepared session=%s temperature=%.3f",
            session_id,
            temperature,
        )
        ...
        try:
            ...
            request = discoveryengine.AnswerQueryRequest(
                ...
                session=self._normalize_session_name(session_id),
            ...
            response = self._search_client.answer_query(
                request=request, retry=retry.Retry(deadline=30.0)
            )

            # Extract answer and grounding
            answer_text = (
                response.answer.answer_text if hasattr(response, "answer") else ""
            )
            citations = []
            grounding_supports = []
            grounding_score = 0.0
            ...
            if hasattr(response.answer, "citations"):
                for cite in response.answer.citations[:max_citations]:
                    src = cite.sources[0] if getattr(cite, "sources", None) else None
                    conf = getattr(cite, "confidence", None)
                    conf_f = self._safe_float(conf, default=0.0)
                    citations.append(
                        {
                            "document_id": getattr(src, "reference_id", "unknown")
                            if src
                            else "unknown",
                            "snippet": getattr(src, "snippet", "") if src else "",
                            "confidence": "high"
                            if conf_f > 0.8
                            else "medium"
                            if conf_f > 0.5
                    ...
                    grounding_supports.append(
                        {
                            ...
                            if hasattr(support, "document_id")
                            else "",
                            "segment": support.segment
                            if hasattr(support, "segment")
                            else "",
                            "score": self._safe_float(
                                getattr(support, "confidence_score", 0.0), default=0.0
                            ),
                        }
                    )
            ...
            if hasattr(response.answer, "grounding_score"):
                grounding_score = self._safe_float(
                    getattr(response.answer, "grounding_score", 0.0), default=0.0
                )
    ...
    def grounded_generation_streaming(
        ...
        try:
            ...
            # Stream response when supported by the installed client library.
            try:
                stream_iter = self._search_client.answer_query(
                    request=request, stream=True
                )
            except TypeError:
                # Fallback: unary call
                stream_iter = [self._search_client.answer_query(request=request)]

            for response in stream_iter:
                if hasattr(response, "answer") and hasattr(
    ...
    def create_session(
        ...
        try:
            ...
            response = self._conversational_search_client.create_session(
                request=request, retry=retry.Retry(deadline=10.0)
            )
    ...
    def update_session(
        ...
        try:
            ...
            response = self._conversational_search_client.update_session(
                request=request
            )
    ...
    def list_sessions(
        ...
        try:
            ...
            sessions = []
            for response in self._conversational_search_client.list_sessions(
                request=request
            ):
                sessions.append(
                    AgentSession(
                        ...
                        name=response.name,
                        state={},
                        created_at=str(response.create_time)
        ...
        *,
        ...
        if not answer_candidate or not answer_candidate.strip():
            raise ValidationError(
                "answer_candidate cannot be empty", field="answer_candidate"
            )
        ...
        try:
            ...
            if not hasattr(self._search_client, "check_grounding"):
                raise ProviderError(
                    "Installed discoveryengine client does not expose check_grounding(); upgrade google-cloud-discoveryengine.",
                    provider="vertex_agent_builder",
                    retryable=False,
                )

            response = self._search_client.check_grounding(
                request=request, retry=retry.Retry(deadline=15.0)
            )

            # Parse response
            is_grounded = (
                bool(response.grounded) if hasattr(response, "grounded") else False
            )
            confidence = (
                float(response.confidence) if hasattr(response, "confidence") else 0.0
            )
    ...
    def rank_documents(
        ...
        *,
        ...
        try:
            ...
            if not hasattr(self._search_client, "rank"):
                raise ProviderError(
                    "Installed discoveryengine client does not expose rank(); upgrade google-cloud-discoveryengine.",
                    provider="vertex_agent_builder",
                    retryable=False,
                )
            ...
            logger.info(
                "rank_documents: input=%d, output=%d", len(documents), len(ranked)
            )
    ...
    def get_datastore_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._datastore_service_client is not None and hasattr(
                self._datastore_service_client, "get_data_store"
            ):
                datastore = self._datastore_service_client.get_data_store(
                    request=request
                )
            elif hasattr(self._document_service_client, "get_data_store"):
                datastore = self._document_service_client.get_data_store(
                    request=request
                )
            else:
                raise ProviderError(
                    "Installed discoveryengine client does not support get_data_store(); upgrade google-cloud-discoveryengine.",
                    provider="vertex_agent_builder",
                    retryable=False,
                )
            ...
            return {
                ...
                if hasattr(datastore, "display_name")
                else "",
                "content_config": str(datastore.content_config)
                if hasattr(datastore, "content_config")
                else "",
                "create_time": str(datastore.create_time)
                if hasattr(datastore, "create_time")
                else "",
            }
    ...
    def multi_turn_search(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                "multi_turn_search: query_len=%d, results=%d", len(query), len(results)
            )
