...
ParticipantDict = dict[str, str]
CommitmentDict = dict[str, Any]
KeyDateDict = dict[str, Any]
NextActionDict = dict[str, Any]
        ...
        if isinstance(obj, list):
            ...
            for item in cast(list[Any], obj):
                if isinstance(item, dict):
                    ...
                    matching_keys = sum(1 for k in known_keys if k in candidate)
                    if matching_keys >= 2:
                        logger.debug(
                        ...
                        return candidate
        obj_type = type(cast(object, obj)).__name__
        logger.warning("JSON parsing: Direct parse gave non-dict: %s", obj_type)
...
def _normalize_name(n: object) -> str:
    """
...
def _normalize_subject_line(s: object) -> str:
    """
...
def _normalize_analysis(data: Any, catalog: Sequence[str] | None) -> JsonDict:
    ...
    catalog_list = list(catalog) if catalog else list(DEFAULT_CATALOG)
    if "other" not in catalog_list:
        catalog_list.append("other")

    d.setdefault("category", catalog_list[-1])
    ...
    if d.get("category") not in catalog_list:
        d["category"] = catalog_list[-1]

    subj = d.get("subject")
    if not isinstance(subj, str):
        subj = "Email thread"
    d["subject"] = _safe_str(_normalize_subject_line(subj), SUBJECT_MAX_LEN)

    participants_raw = d.get("participants")
    if isinstance(participants_raw, list):
        ...
        parts: list[ParticipantDict] = []
        seen_keys: set[str] = set()
        participant_list = cast(list[Any], participants_raw)
        for item in participant_list:
            if not isinstance(item, dict):
                continue
            participant = cast(JsonDict, item)
            name = _safe_str(participant.get("name"), 80)
            email = _safe_str(participant.get("email"), 120)
            role_raw = participant.get("role")
            tone_raw = participant.get("tone")
            role = (
                role_raw
                if isinstance(role_raw, str) and role_raw in allowed_roles
                else "other"
            )
            tone = (
                tone_raw
                if isinstance(tone_raw, str) and tone_raw in allowed_tones
                else "neutral"
            )
            stance = _safe_str(participant.get("stance", "N/A"), 200)
            if not name and not email:
                continue
            norm: ParticipantDict = {
                "name": name,
                "role": role,
                "email": email,
                "tone": tone,
            ...
            key = email_key or (f"name:{name_key}" if name_key else "")
            if key:
                if key in seen_keys:
                    continue
                seen_keys.add(key)
            parts.append(norm)
        d["participants"] = parts[:MAX_PARTICIPANTS]
    else:
        d["participants"] = []

    summary_raw = d.get("summary")
    if isinstance(summary_raw, list):
        summary_list = cast(list[Any], summary_raw)
        summary_items = [str(item) for item in summary_list if isinstance(item, str)]
        d["summary"] = summary_items[:MAX_SUMMARY_POINTS]
    elif d.get("summary"):
        d["summary"] = [str(d.get("summary"))]
    else:
        d["summary"] = []
    ...
    if isinstance(next_actions_raw, list):
        ...
        coerced_actions: list[NextActionDict] = []
        actions_list = cast(list[Any], next_actions_raw)
        for item in actions_list:
            if not isinstance(item, dict):
                continue
            action = cast(JsonDict, item)
        ...
        d["next_actions"] = coerced_actions[:MAX_NEXT_ACTIONS]
    else:
        d["next_actions"] = []

    risks_raw = d.get("risk_indicators")
    if isinstance(risks_raw, list):
        risk_list = cast(list[Any], risks_raw)
        d["risk_indicators"] = [
            str(item) for item in risk_list if isinstance(item, str)
        ]
    else:
        d["risk_indicators"] = []

    ledger_raw = d.get("facts_ledger")
    ledger: JsonDict = (
        dict(cast(JsonDict, ledger_raw)) if isinstance(ledger_raw, dict) else {}
    )
    ...
    for key in default_ledger_keys:
    ...
    commitments_raw = ledger.get("commitments_made", [])
    coerced_commits: list[CommitmentDict] = []
    if isinstance(commitments_raw, list):
        commitments_list = cast(list[Any], commitments_raw)
        for item in commitments_list:
            if not isinstance(item, dict):
                continue
            commit = cast(JsonDict, item)
    ...
    ledger["commitments_made"] = coerced_commits[:MAX_FACT_ITEMS]

    key_dates_raw = ledger.get("key_dates", [])
    coerced_dates: list[KeyDateDict] = []
    if isinstance(key_dates_raw, list):
        key_dates_list = cast(list[Any], key_dates_raw)
        for item in key_dates_list:
            if not isinstance(item, dict):
                continue
            kd = cast(JsonDict, item)
    ...
    ledger["key_dates"] = coerced_dates[:MAX_FACT_ITEMS]

    for key in default_ledger_keys:
        items = ledger.get(key, [])
        if isinstance(items, list):
            ledger[key] = items[:MAX_FACT_ITEMS]
        else:
            ledger[key] = []

    d["facts_ledger"] = ledger

    return d


def _merge_manifest_into_analysis(
    analysis: Any, convo_dir: object, raw_thread_text: str | None
) -> JsonDict:
    """Merge manifest metadata into an LLM analysis dict."""
    ...
    analysis_dict: JsonDict = dict(cast(JsonDict, analysis))

    if not isinstance(convo_dir, Path):
        logger.error("_merge_manifest_into_analysis: convo_dir is not a Path")
        return analysis_dict

    raw_text = raw_thread_text or ""

    manifest_obj = load_manifest(convo_dir)
    manifest: JsonDict | None = (
        dict(cast(JsonDict, manifest_obj)) if isinstance(manifest_obj, dict) else None
    )

    existing_subject = analysis_dict.get("subject")
    ...
    if manifest:
        smart = _safe_str(manifest.get("smart_subject"), SUBJECT_MAX_LEN).strip()
        if smart:
            subj_candidates.append(smart)
        subj = _safe_str(manifest.get("subject"), SUBJECT_MAX_LEN).strip()
        if subj:
    ...
    if isinstance(metadata, dict):
        ...
        if subject_val:
            subj_candidates.append(str(subject_val).strip())

    final_subject = subj_candidates[0] if subj_candidates else "Email thread"
    analysis_dict["subject"] = _safe_str(
        _normalize_subject_line(final_subject), SUBJECT_MAX_LEN
    )

    participants_raw = analysis_dict.get("participants")
    existing_participants: list[JsonDict] = (
        cast(list[JsonDict], participants_raw)
        if isinstance(participants_raw, list)
        else []
    )

    manifest_participants_raw = (
        extract_participants_detailed(manifest) if manifest else []
    )
    manifest_participants: list[JsonDict] = (
        cast(list[JsonDict], manifest_participants_raw)
        if isinstance(manifest_participants_raw, list)
        else []
    )

    seen_keys: set[str] = set()
    merged_participants: list[JsonDict] = []

    for participant in existing_participants:
        if isinstance(participant, dict):
            email_key = _safe_str(participant.get("email"), 120).lower()
    ...
    for participant in manifest_participants:
        if isinstance(participant, dict):
            email_key = _safe_str(participant.get("email"), 120).lower()
            name_key = _normalize_name(participant.get("name", ""))
            if not email_key and not name_key:
    ...
    analysis_dict["participants"] = merged_participants[:MAX_PARTICIPANTS]

    ledger_raw = analysis_dict.get("facts_ledger")
    ledger: JsonDict = (
        dict(cast(JsonDict, ledger_raw)) if isinstance(ledger_raw, dict) else {}
    )

    key_dates_raw = ledger.get("key_dates")
    existing_dates: list[JsonDict] = (
        cast(list[JsonDict], key_dates_raw) if isinstance(key_dates_raw, list) else []
    )

    manifest_dates: list[JsonDict] = []
    if manifest:
        time_span_raw = manifest.get("time_span")
        time_span = (
            cast(JsonDict, time_span_raw) if isinstance(time_span_raw, dict) else {}
        )
    ...
    merged_dates: list[JsonDict] = []

    for date_entry in existing_dates:
        if isinstance(date_entry, dict):
            date_key = (
                str(date_entry.get("date", "")).lower(),
                str(date_entry.get("event", "")).lower(),
            )
            if date_key not in seen_date_keys:
                seen_date_keys.add(date_key)
                merged_dates.append(date_entry)

    for date_entry in manifest_dates:
        date_key = (
            str(date_entry.get("date", "")).lower(),
            str(date_entry.get("event", "")).lower(),
        )
    ...
    ledger["key_dates"] = merged_dates[:MAX_FACT_ITEMS]
    analysis_dict["facts_ledger"] = ledger
...
async def _retry(
    callable_fn: Any, *args: Any, retries: int = 2, delay: float = 0.5, **kwargs: Any
) -> Any:
    """
