...
from .utils import read_text_file, strip_control_chars
...
_extraction_cache: dict[
    tuple[Path, int | None], tuple[float, float, int, str]
] = {}  # (cached_at, mtime, size, text)
_extraction_cache_lock = threading.Lock()
_CACHE_TTL = int(
    os.getenv("EXTRACTION_CACHE_TTL", "3600")
...
def _is_allowed_by_patterns(path: Path, patterns: Sequence[str] | None) -> bool:
    """
    Return True if the path matches at least one of the allowed patterns.

    Supports common pattern styles:
      - Extension strings: ".pdf", "pdf"
      - Glob patterns: "*.pdf", "**/*.pdf"
    """
    if not patterns:
        return True
    ...
    for pat in patterns:
        ...
        # Extension-style patterns
        if p.startswith(".") and not any(ch in p for ch in "*?[]/\\"):
            if suffix == p.lower():
                return True
            continue
        if (
            not any(ch in p for ch in "*?[]")
            and not p.startswith(".")
            and "/" not in p
            and "\\" not in p
        ):
            if suffix == ("." + p.lower().lstrip(".")):
                return True
            continue
        ...
        try:
            if path.match(p):
                return True
            if not p.startswith("**/") and path.match("**/" + p):
                return True
        except Exception:
            continue
...
def extract_text(
    path: Path, *, max_chars: int | None = None, use_cache: bool = True
) -> str:
    """
    ...
    """
    # P2-2 FIX: Load config once and reuse
    ...
    allowed_patterns = getattr(
        getattr(cfg, "file_patterns", None), "allowed_file_patterns", None
    )
    if not _is_allowed_by_patterns(path, allowed_patterns):
