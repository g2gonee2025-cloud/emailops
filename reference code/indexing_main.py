...
def _get_index_limits() -> tuple[float, int]:
    try:
        ...
        return float(cfg.limits.max_indexable_file_mb), int(
            cfg.limits.max_indexable_chars
        )
    except Exception:
...
def _materialize_text_for_docs(
    docs: list[dict[str, Any]], file_service: FileService
) -> list[dict[str, Any]]:
    """
...
# ============================================================================
def _load_pre_chunked_data(
    chunk_file: Path, base_id: str, conv_mtime: float, limit: int | None
) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(chunks_data, list):
            ...
            upto = (
                len(chunks_data)
                if limit is None
                else max(0, min(limit, len(chunks_data)))
            )
...
def _build_doc_entries(
    ...
    Supports two modes:
      1) Pre-chunked mode: if `_chunks/{base_id}.json` exists, load it.
         (Chunk files are expected to contain cleaned text and stable IDs.)
      2) Fallback mode: if chunk file is missing/invalid, perform fresh clean+chunk.

    Always attempts to include attachments (even when pre-chunked conversation chunks exist),
    so a full rebuild indexes both the conversation thread and its files.

    Args:
        convo_dir: Conversation folder
        base_id: Conversation ID (folder name)
        limit: Optional maximum number of produced chunks for this conversation
        metadata: Lightweight metadata from manifest (subject, from/to/cc, start/end date)

    Returns:
        List of chunk dicts with required keys: id, text, path, doc_type, conv_id, modified_time
    """
    ...
    try:
        conv_mtime = (
            float(conv_file.stat().st_mtime)
            if conv_file.exists()
            else float(time.time())
        )
    except Exception:
        conv_mtime = float(time.time())

    docs: list[dict[str, Any]] = []

    # ----------------------------
    # 1) Load pre-chunked conversation chunks (if present)
    # ----------------------------
    chunks_dir = convo_dir.parent / "_chunks"
    chunk_file = chunks_dir / f"{base_id}.json"
    if chunk_file.exists():
        pre = _load_pre_chunked_data(chunk_file, base_id, conv_mtime, limit=None)
        if pre:
            # Backfill metadata fields if missing
            for d in pre:
                if not isinstance(d, dict):
                    continue
                d.setdefault("conv_id", base_id)
                d.setdefault("doc_type", d.get("doc_type", "conversation"))
                d.setdefault("modified_time", conv_mtime)
                for k in ("subject", "from", "to", "cc", "start_date", "end_date"):
                    if k in meta and (k not in d or d.get(k) in (None, "", [])):
                        d[k] = meta[k]
            docs.extend(pre)

    # Determine whether attachments are already included in the chunk file
    have_attachment_docs = any(
        str(d.get("doc_type") or "").lower() == "attachment"
        for d in docs
        if isinstance(d, dict)
    )
    have_conversation_docs = any(
        str(d.get("doc_type") or "").lower() == "conversation"
        for d in docs
        if isinstance(d, dict)
    )

    # ----------------------------
    # 2) Fallback: fresh conversation chunking when needed
    # ----------------------------
    conv_data: dict[str, Any] | None = None
    if not have_conversation_docs:
        try:
            conv_data = (
                load_conversation(
                    convo_dir,
                    include_attachment_text=False,
                    max_total_attachment_text=None,
                )
                or {}
            )
        except Exception:
        ...
        raw_text = (
            conv_data.get("conversation_txt") if isinstance(conv_data, dict) else ""
        )
        ...
        if cleaned.strip():
            ...
            date_hint = meta.get("end_date") or meta.get("start_date")
            chunks = prepare_index_units(
                text=cleaned,
                doc_id=conv_doc_id,
                doc_path=str(conv_file),
                subject=str(meta.get("subject") or ""),
                date=str(date_hint) if date_hint else None,
            )
            for ch in chunks:
                ch.update(
                    {
    ...
    # ----------------------------
    # 3) Attachments (always include on full rebuild, unless already present)
    # ----------------------------
    if not have_attachment_docs:
        # Load conversation data once if we didn't already
        ...
        max_file_mb, max_text_chars = _get_index_limits()

        # Prefer attachments provided by loader (may already include extracted text)
        attachment_items: list[dict[str, Any]] = []
        try:
            raw_atts = (
                (conv_data or {}).get("attachments")
                if isinstance(conv_data, dict)
                else None
            )
            if isinstance(raw_atts, list):
                attachment_items = [a for a in raw_atts if isinstance(a, dict)]
        except Exception:
            attachment_items = []

        # Also scan the filesystem to avoid missing attachments
        att_paths_from_fs = list(_iter_attachment_files(convo_dir))
        seen_paths: set[Path] = set()

        def _resolve_path(p: Path) -> Path:
            try:
                if not p.is_absolute():
                    return (convo_dir / p).resolve()
                return p.resolve()
            except Exception:
                return p

        # Build an iterator of (path, text_optional)
        ...
        for a in attachment_items:
            ...
            if not p_raw:
                continue
            p = _resolve_path(Path(str(p_raw)).expanduser())
            if p in seen_paths:
                continue
            seen_paths.add(p)
            txt = a.get("text")
            candidates.append((p, txt if isinstance(txt, str) else None))

        for p in att_paths_from_fs:
            rp = _resolve_path(p)
            if rp in seen_paths:
                continue
            seen_paths.add(rp)
            candidates.append((rp, None))

        # Extraction helper for attachments that don't have loader-provided text
        extract_text_fn = None
        with contextlib.suppress(Exception):
            # package import
            from .core_text_extraction import (
                extract_text as extract_text_fn,  # type: ignore
            )
        ...
        for att_path, maybe_text in candidates:
            try:
                if not att_path.exists() or not att_path.is_file():
                    continue

                # file size guard
                size_mb = float(att_path.stat().st_size) / (1024.0 * 1024.0)
                if size_mb > float(max_file_mb):
                    logger.debug(
                        "Skipping attachment over size limit (%.1f MB > %.1f MB): %s",
                        size_mb,
                        float(max_file_mb),
                        att_path,
                    )
                ...
                att_mtime = float(att_path.stat().st_mtime)

                text = maybe_text or ""
                if not text.strip() and extract_text_fn is not None:
                    # Fallback to on-disk extraction for common formats
                    text = extract_text_fn(att_path, max_chars=int(max_text_chars))  # type: ignore[misc]
                ...
                doc_id = _att_id(base_id, att_path)
                date_hint = meta.get("end_date") or meta.get("start_date")
                chunks = prepare_index_units(
                    text=text[: int(max_text_chars)] if max_text_chars else text,
                    doc_id=doc_id,
                    ...
                    ch.update(
                        {
                            ...
                            "attachment_type": att_path.suffix.lstrip(".").lower(),
                        }
                    )
                    for k in ("subject", "from", "to", "cc", "start_date", "end_date"):
                        if k in meta:
                ...
                logger.debug("Failed processing attachment %s: %s", att_path, e)

    # Enforce per-conversation limit (across conversation + attachments)
                ...
                with safe_load_array(ixp.embeddings, mmap_mode="r") as emb:
                    ...
                    if HAVE_FAISS and faiss is not None and ixp.faiss.exists():
                        try:
                            ...
                            raise RuntimeError(
                                f"FAISS consistency check failed: {e}"
                            ) from None
            except FileNotFoundError:
                # Embeddings file removed between check and load
                raise RuntimeError(
                    "Embeddings file not found during consistency check"
                ) from None
    except Exception:
        raise
    ...
    *,
    ...
    if HAVE_FAISS and faiss is not None:
        try:
            ...
            logger.warning(
                "FAISS indexing failed, continuing without FAISS optimization: %s", e
            )
